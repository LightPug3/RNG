...
void vButtonCheck( void * pvParameters )  {
    ...
    for( ;; ) {
        // Add code here to check if a button(S) is pressed then execute appropriate function if a button is pressed  
        if(digitalRead(BTN_A) == LOW){
          delay(100);                         // debouncing
          Serial.println("Button pressed!");  // console message
          GDP();                              // run GDP function
        }
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(type, "toggle") == 0){
    ...
    doc["timestamp"]  = getTimeStamp();
    /*Add code here to insert all other variabes that are missing from Json object according to schema above */
    doc["number"]     = number;
    doc["ledA"]       = getLEDStatus(LED_A);
    doc["ledB"]       = getLEDStatus(LED_B);
...
void GDP(void){
  ...
  // Add key:value pairs to Json object according to below schema
  // ‘{"id": "student_id", "timestamp": 1702212234, "number": 9, "ledA": 0, "ledB": 0}’
  doc["id"]         = "620156694"; // Change to your student ID number
  doc["timestamp"]  = getTimeStamp();
  /*Add code here to insert all other variabes that are missing from Json object according to schema above*/
  doc["number"]     = number;
  doc["ledA"]       = getLEDStatus(LED_A);
